...
class TestLimitOffset:
    ...
    async def test_limit(self, db):
        posts = await Post.objects.order_by("id").limit(2).all(client=db)
        assert len(posts) == 2
        assert posts[0].id == 1
        assert posts[1].id == 2
    ...
    async def test_offset(self, db):
        ...
        posts = await Post.objects.order_by("id").limit(100).offset(4).all(client=db)
        assert len(posts) == 2  # posts 5 and 6

    @pytest.mark.asyncio
    async def test_limit_offset(self, db):
        posts = await Post.objects.order_by("id").limit(2).offset(1).all(client=db)
        assert len(posts) == 2
        assert posts[0].id == 2
        assert posts[1].id == 3
    ...
    async def test_slicing(self, db):
        posts = await Post.objects.order_by("id")[1:3].all(client=db)
        assert len(posts) == 2
        assert posts[0].id == 2
        assert posts[1].id == 3
...
class TestOrderBy:
    ...
    async def test_order_by_asc(self, db):
        authors = await Author.objects.order_by("name").all(client=db)
    ...
    async def test_order_by_multiple(self, db):
        ...
        authors = await Author.objects.order_by("active", "-name").all(client=db)
        names = [a.name for a in authors]
...
class TestDistinct:
    ...
    async def test_distinct(self, db):
        author_ids = await (
            Post.objects.values_list("author_id", flat=True).distinct().all(client=db)
        )
...
class TestValues:
    ...
    async def test_values(self, db):
        rows = await Author.objects.order_by("id").values("id", "name").all(client=db)
        assert len(rows) == 3
...
class TestCountExists:
    ...
    async def test_count(self, db):
        count = await Post.objects.count(client=db)
        assert count == 6

    @pytest.mark.asyncio
    async def test_exists_true(self, db):
        exists = await Post.objects.filter(published=True).exists(client=db)
        assert exists is True

    @pytest.mark.asyncio
    async def test_exists_false(self, db):
        exists = await Post.objects.filter(title="Nonexistent").exists(client=db)
...
class TestFirstLast:
    ...
    async def test_first(self, db):
        post = await Post.objects.order_by("id").first(client=db)
        assert post is not None
        assert post.id == 1

    @pytest.mark.asyncio
    async def test_last(self, db):
        post = await Post.objects.order_by("id").last(client=db)
        assert post is not None
        assert post.id == 6
